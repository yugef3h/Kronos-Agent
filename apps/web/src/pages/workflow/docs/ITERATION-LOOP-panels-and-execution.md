...
- 当前轻量版 workflow 仍然没有完整的容器子图编辑器。
- 所以这里优先保证的是“后端真正依赖的 DSL 字段完整可持久化”，而不是复刻 Dify 那套完整的内部 start node 画布交互。

换句话说，当前实现已经能把容器配置正确保存下来并让前后端工程正常构建运行，但如果后续要继续逼近文档里的完整形态，下一步应该补的是 iteration / loop 的内部子图编辑能力。