...
const IterationPanel = ({ id, data }: NodePanelProps) => {
  ...
  const iteratorOption = useMemo(
    () =>
      findVariableOption(
        (nodeData.inputs as IterationNodeConfig | undefined)?.iterator_selector ?? [],
        variableOptions,
      ),
    ...
    () =>
      variableOptions.filter(
        (option) =>
          ...
          serializeValueSelector(option.valueSelector) === 'sys.files',
      ),
    [variableOptions],
  );
    ...
    <div className="space-y-3">
      ...
      {activeTab === 'settings' ? (
        <>
          ...
          <PanelSection title="输入" required>
            {issues.length ? <PanelAlert type="warning">{issues[0].message}</PanelAlert> : null}
            <PanelCard className="space-y-2.5 bg-white p-0 shadow-none border-none">
              <Field title="" compact>
                <VariableSelect
                  ...
                  onChange={(valueSelector) =>
                    handleIteratorSelectorChange(
                      valueSelector,
                      serializeValueSelector(valueSelector) === `${id}.item`
                        ? currentItemOption
                        : findVariableOption(valueSelector, iteratorCandidates),
                    )
                  }
                />
              </Field>
          ...
          <PanelSection title="聚合输出" required>
