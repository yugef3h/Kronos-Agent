...
describe('takeoutOrchestratorService parser', () => {
  ...
  it('delegates non-takeout prompts without markers', () => {
    const result = parseTakeoutOrchestrationOutput('今天 AI 行业有几条新闻。', '今天 AI 行业有几条新闻');

    expect(result.action).toBe('delegate_chat_stream');
