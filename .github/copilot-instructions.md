...
### AI 行为
- **增量提交**：每完成一小块可独立通过校验的改动就提交一次，多个功能要拆开提交；只 `git add` 本轮文件，在根目录执行 `pnpm cd "feat|fix|docs|test: 一句话"`（与 `pnpm run cd` 等价）。勿用 `pnpm cd -- "…"`，否则 `--` 会进 commit message。`git push` 不默认代劳。
...
- 如果有问题或未理解到位的，或被指出的差异，记录文档 ./DEBRIED_ISSUES.md，方便复盘
