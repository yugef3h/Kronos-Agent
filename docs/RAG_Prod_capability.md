...
- RAG 可人工做测试集标注、人工统计召回率；**无内置 EM/F1/幻觉率 自动量化评估指标**
  - **Kronos（免费、无 LLM 裁判）**：`POST /api/workflow/knowledge-retrieval/evaluate` 对多条用例复用同一套检索参数 `shared`，逐条调用现有检索；可选 `gold_chunk_ids` 得 **Recall@K / MRR**；可选 `expected_answer` 得 **EM / 字级 F1**（预测文本为 TopK chunk 正文拼接，非真实生成器输出）；可选 `generated_answer` 得 **证据外字符占比**（启发式，不等价于业界「幻觉率」）。控制台可用 `requestKnowledgeRetrievalEvaluate` 调用。
